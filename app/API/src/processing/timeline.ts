...
export function formatTimeline(timelineData) {
  const output = {};
  ...
  output.information = {
    ...
    location: `${timelineData.sport_event.venue?.city_name ?? 'Unknown'}, ${
      timelineData.sport_event.venue?.country_name ?? 'Unknown'
    }`,
